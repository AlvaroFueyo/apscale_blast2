"""Database builder: PR2.

Builds a local BLAST database from a PR2 FASTA release and extracts taxonomy
...
def pr2_taxonomy_table(fasta_path: str) -> pd.DataFrame:
    """Parse PR2 headers into a taxonomy table.

    Replicates Till's logic:
      taxonomy_dict = {k:v for k:v in record.id.split(';')[1].split('=')[1].split(',')}
    """
    rows = []
    for hdr in _iter_fasta_headers(fasta_path):
        token = hdr.split()[0]
        accession = token.split(";", 1)[0]

        tax_dict = {}
        parts = token.split(";")
        seg = parts[1] if len(parts) > 1 else ""
        if not seg.startswith("tax="):
            raise ValueError("Unsupported PR2 FASTA dialect; use the official UTAX FASTA release")
        if "=" in seg:
            seg = seg.split("=", 1)[1]
        ...
        if not any(taxonomy):
            raise ValueError(f"PR2 header has no recognized ranks: {token}")
...
def build_pr2_db(
    *,
    ...
    """Build a PR2 BLAST database folder and return its path."""
    if not name:
    ...
    try:
        ...
        tax_df = pr2_taxonomy_table(resolved)
